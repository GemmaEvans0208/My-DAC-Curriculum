...
-- SELECT DISTINCT values: Unique column value, removes "duplicated records" in a result set by keeping one row
...
-- From employee table, ORDER BY hiredate, ASC(ascending) and DESC(descending)
...
FROM production.product AS product -- Left Table(aft 'FROM')
INNER JOIN production.productsubcategory AS productsubcategory -- Right Table
        ON product.productsubcategoryid = productsubcategory.productsubcategoryid
INNER JOIN production.productcategory AS productcategory -- Add to Right Table of the intersect of previous two tables
...
-- common column: businessentityid
...
-- A6:  
/* It depends on the relationship that both table present for the left join share.
...
SELECT 
	...
	CAST(person.businessentityid AS numeric)/2 AS numeric,
	CAST(person.businessentityid AS decimal)/2 AS decimal,
...
--CASE FUNCTION: CASE WHEN THEN ELSE END (like 'if' function in py)
